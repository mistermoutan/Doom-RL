...
class Worker():
    ...
    def __init__(self, name, s_size, a_size, trainer=None, model_path=None, player_mode=False):
        ...
        if not player_mode:
            ...
            self.summary_writer = tf.summary.FileWriter(params.summary_path+"/train"+str(self.number))
    ...
    def print_end_episode_perfs(self):
        ...
        if params.scenario=='basic':
            print('{}, episode #{}, ep_reward: {}, av_reward:{}, steps:{}, time costs:{}'.format(
                            self.name, self.episode_count, self.episode_reward, self.episode_reward/self.episode_step_count, 
                            self.episode_step_count, time.time()-self.episode_st))

        if params.scenario=='deadly_corridor':
            print('{}, health: {}, kills:{}, episode #{}, ep_reward: {}, av_reward:{}, steps:{}, time costs:{}, time stamp:{}'.format(
                            self.name, np.maximum(0,self.last_total_health), self.last_total_kills, self.episode_count,
                            self.episode_reward, self.episode_reward/self.episode_step_count, 
                            self.episode_step_count, time.time()-self.episode_st, datetime.now().time()))

        if params.scenario=='defend_the_center':
            print('{}, kills:{}, episode #{}, ep_reward: {}, av_reward:{}, steps:{}, time costs:{}'.format(
                            self.name, self.last_total_kills, self.episode_count, self.episode_reward, 
                            self.episode_reward/self.episode_step_count, 
                            self.episode_step_count, time.time()-self.episode_st))

        if params.scenario=='health_gathering':
            print('{}, episode #{}, ep_reward: {}, health: {} av_reward:{}, steps:{}, time costs:{}'.format(
                            self.name, self.episode_count, self.episode_reward, self.last_total_health, 
                            self.episode_reward/self.episode_step_count,  
                            self.episode_step_count, time.time()-self.episode_st))
    ...
    def work(self,max_episodes,gamma,sess,coord,saver):
        stats = Statistics(scenario, "a3c", max_episodes, "/home/piotr/Projects/Doom_A3C/statistics/" + str(params.scenario) + "/")
        ...
        with sess.as_default(), sess.graph.as_default():
            while(not coord.should_stop()) and (self.episode_count < max_episodes):
                ...
                while is_episode_done == False:
                    ...
                    s10 = process_frame(s10,crop,resize)
                    s11 = process_frame(s11,crop,resize)
                    s1 = np.append(s10,s11)
                ...
                if (self.episode_count % 100 == 0 or (params.scenario == "deadly_corridor" and self.last_total_kills > 4) or (params.scenario == "defend_the_center" and self.last_total_kills > 20) or (params.scenario == "health_gathering" and self.episode_step_count > 700)):
                    format_frames = np.array(episode_frames)
                    format_frames2 = []
                    for i in range(format_frames.shape[0]):
                        format_frames2.append(format_frames[i,:,:,0])
                        format_frames2.append(format_frames[i,:,:,1])
                    imageio.mimwrite('videos/' + str(params.scenario) + '/'+str(self.name)+ "_" +str(self.episode_count)+'.mp4', format_frames[:,:,:,0], fps = 15)
                    format_frames2 = []

                    print("video exported")
                episode_frames = []

                # append to stats class
                stats.kills_per_episode.append(self.last_total_kills)
                stats.rewards_per_episode.append(self.episode_reward)
                stats.length_episodes.append(self.episode_step_count)
                stats.episode_per_epoch.append(1)
                stats.loss_actor.append(self.p_l)
                stats.loss_critic.append(self.v_l)
